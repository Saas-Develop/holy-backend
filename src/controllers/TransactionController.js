...
export const getTransactionsByYearAndMonth = async (req, res) => {
    const userId = req._id
    ...
    if (!year || !month) {
        return res.status(400).json({ msg: 'Ano ou mês não fornecido.' })
    }
    ...
    try {
        const transactions = await Transaction.find({ 
            user: userId, 
            date: { 
                $regex: new RegExp(`/${month.padStart(2, '0')}/${year}$`, "i") 
            } 
        })
